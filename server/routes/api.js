...
router.get('/community/questions', (req, res) => {
  ...
  if (sort === 'votes') orderBy = 'score DESC, q.created_at DESC';
...
router.post('/community/questions/:questionId/vote', (req, res) => {
  const { direction, isQuestion } = req.body;
  ...
  const targetId   = isQuestion ? req.params.questionId : req.params.questionId; // answer votes use same route with answer id in body

  // For anonymous voting we use a session-based or IP-based approach
  // For simplicity: use a per-target per-session vote (no user auth needed)
  const existing = queryOne('SELECT * FROM votes WHERE user_id=? AND target_type=? AND target_id=?',
    ['anonymous', targetType, req.params.questionId]);
...
router.post('/questions/:id/vote', authenticate, (req, res) => {
  ...
  const existing = queryOne('SELECT * FROM votes WHERE user_id=? AND target_type=? AND target_id=?',
    [req.user.id, 'question', question.id]);
  ...
  if (existing) {
    if (existing.direction === direction) {
      ...
      run('UPDATE users SET reputation = reputation + ? WHERE id = ?', [deltaMap[direction].same, question.user_id]);
    } else {
      run('UPDATE votes SET direction = ? WHERE id = ?', [direction, existing.id]);
      run('UPDATE users SET reputation = reputation + ? WHERE id = ?', [deltaMap[direction].flip, question.user_id]);
    }
  } else {
    run('INSERT INTO votes (id, user_id, target_type, target_id, direction) VALUES (?, ?, ?, ?, ?)',
      [uuidv4(), req.user.id, 'question', question.id, direction]);
    run('UPDATE users SET reputation = reputation + ? WHERE id = ?', [direction === 'up' ? 10 : -2, question.user_id]);
  }
...
router.post('/answers/:id/vote', authenticate, (req, res) => {
  ...
  if (existing) {
    if (existing.direction === direction) {
      run('DELETE FROM votes WHERE id = ?', [existing.id]);
      run('UPDATE users SET reputation = reputation + ? WHERE id = ?', [deltaMap[direction].same, answer.user_id]);
    } else {
      run('UPDATE votes SET direction = ? WHERE id = ?', [direction, existing.id]);
      run('UPDATE users SET reputation = reputation + ? WHERE id = ?', [deltaMap[direction].flip, answer.user_id]);
    }
  } else {
    run('INSERT INTO votes (id, user_id, target_type, target_id, direction) VALUES (?, ?, ?, ?, ?)',
      [uuidv4(), req.user.id, 'answer', answer.id, direction]);
    run('UPDATE users SET reputation = reputation + ? WHERE id = ?', [direction === 'up' ? 10 : -2, answer.user_id]);
  }

  const upvotes   = queryOne('SELECT COUNT(*) as c FROM votes WHERE target_type=? AND target_id=? AND direction=?', ['answer', answer.id, 'up'])?.c || 0;
  const downvotes = queryOne('SELECT COUNT(*) as c FROM votes WHERE target_type=? AND target_id=? AND direction=?', ['answer', answer.id, 'down'])?.c || 0;
  res.json({ upvotes, downvotes, score: upvotes - downvotes });
});
